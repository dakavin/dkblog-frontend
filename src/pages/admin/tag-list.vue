...
function getTableData() {
    ...
    getTagPageList({
        ...
        endDateTime: endDate.value, name: searchTagName.value
    }).then((res) => {
        if (res.success === true) {
            ...
            <div class="flex flex-col md:flex-row items-center space-y-3 md:space-y-0 md:space-x-5">
                ...
                <div class="flex flex-col md:flex-row md:items-center w-full">
                    <div class="flex items-center md:w-auto ml-5">
                        <el-text class="text-left w-13 hide-on-mobile">标签名称</el-text>
                        <el-input v-model="searchTagName" placeholder="请输入标签名称（支持模糊查询）" class="ml-3" style="width: 300px;"/>
                    </div>
                </div>
